...
def execute_command():
    ...
    # Disable desktop automation based on environment variable
    ...
    if intent in desktop_automation_intents and not enable_desktop_automation:
        return jsonify({
            "success": False,
...
def open_file_cross_platform(path):
    """Open a file or URL using the appropriate method for the current OS."""
    import platform
    import subprocess
    ...
    if platform.system() == 'Windows':
        try:
            ...
            # Fallback for Windows if startfile is not available
            try:
...
def launch_windows_target(target):
    ...
    if normalized.endswith(":"):
        try:
            open_file_cross_platform(normalized)
            return
        except OSError:
...
def handle_open_folder(params):
    ...
    try:
        ...
        if folder in folder_paths:
            open_file_cross_platform(folder_paths[folder])
...
def handle_file_system(params):
    ...
    try:
        if action == "list_files":
            ...
            open_file_cross_platform(os.getcwd())
